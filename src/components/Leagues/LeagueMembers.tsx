  ...
  if (error) {
    return (
      <div className="text-center py-12 bg-red-50 border border-red-200 rounded-lg p-6">
        ...
        <p className="text-red-600 font-semibold">Erro ao carregar membros da liga:</p>
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-6 border">
      <div className="flex items-center mb-6">
        ...
        <h2 className="text-xl font-bold text-gray-800">Membros da Liga: {league.name}</h2>
      </div>

      {members.length === 0 ? (
        <p className="text-gray-500 text-center py-4">Nenhum membro encontrado para esta liga.</p>
      ) : (
        <ul className="space-y-4">
          {members.map((member) => (
            <li key={member.id} className="flex items-center p-3 bg-gray-50 rounded-md hover:bg-gray-100 transition-colors">
              {member.avatar ? (
