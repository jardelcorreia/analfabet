...
const dbHelpers = {
  ...
  // Real-time score update with immediate bet calculation
  async updateMatchScores(id, homeScore, awayScore, status) {
    const result = await sql`
      UPDATE matches
      SET home_score = ${homeScore}, away_score = ${awayScore}, status = ${status}, updated_at = CURRENT_TIMESTAMP
      WHERE id = ${id}
      RETURNING *
    `;
    
    if (result[0]) {
      // Immediately calculate bet points for this match
      await this.calculateBetPointsForMatch(id, homeScore, awayScore);
    }
    
    return result[0];
  },

  // Calculate bet points for a specific match (real-time)
  async calculateBetPointsForMatch(matchId, homeScore, awayScore) {
    ...
    const result = await sql`
      UPDATE bets SET
        points = CASE
          ...
          WHEN bets.home_score = ${homeScore} AND bets.away_score = ${awayScore} THEN 3
          -- Resultado correto: 1 ponto
          WHEN (
            -- Vitória do mandante
            (bets.home_score > bets.away_score AND ${homeScore} > ${awayScore}) OR
            -- Vitória do visitante
            (bets.home_score < bets.away_score AND ${homeScore} < ${awayScore}) OR
            -- Empate
            (bets.home_score = bets.away_score AND ${homeScore} = ${awayScore})
          ) THEN 1
          -- Resultado errado: 0 pontos
          ELSE 0
        END,
        is_exact = (bets.home_score = ${homeScore} AND bets.away_score = ${awayScore}),
        updated_at = CURRENT_TIMESTAMP
      WHERE bets.match_id = ${matchId}
      RETURNING *
    `;

    // Update user stats for all affected users
    const affectedLeagues = await sql`
      SELECT DISTINCT league_id FROM bets WHERE match_id = ${matchId}
    `;

    for (const league of affectedLeagues) {
      await this.recalculateUserStatsForLeague(league.league_id);
    }

    return result;
  },

  // Recalculate user stats for a specific league
  async recalculateUserStatsForLeague(leagueId) {
    const users = await sql`
      SELECT DISTINCT user_id FROM bets WHERE league_id = ${leagueId}
    `;
    ...
    for (const user of users) {
      await sql`
        ...
        FROM bets b
        WHERE b.user_id = ${user.user_id} AND b.league_id = ${leagueId}
